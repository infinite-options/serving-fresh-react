...
function Cart() {
  ...
  const endpoint_Coupon = "https://tsx3rnuidi.execute-api.us-west-1.amazonaws.com/dev/api/v2/available_Coupons/xyz@gmail.com";
  ...
  function stripePay(){
    ...
    if(cartContext.cartTotal > 0){
      const postInfo = {
        "pur_customer_uid" : "100-000009",
        "pur_business_uid" : allItemsHolder[0].business_uid,
        "items" : arrayOfItem,
  ...
  var shippingCost =1.5;
  var shipFee = (Math.floor(shippingCost * 100) / 100).toFixed(2);
  ...
  //the price need to pay
  var finalPrice= (Math.floor(((tax * subprice) + shippingCost + subprice - (couponValue * subprice)) * 100) / 100).toFixed(2);
  ...
  const [couponLoaded, setCouponStatus] = useState(false);
  const [couponError, setErrorStatus] = useState(false);
  const [couponData, setCouponData] = useState([]);

  useEffect(() => {
    axios.get(endpoint_Coupon).then(response =>{
      var itemPack = response.data.result;
      var validCoupons =[];
      for(var i=0;i<itemPack.length;i++){
        if(itemPack[i].valid ==="TRUE"){
          validCoupons.push(itemPack[i]);
        }
      }
